...
def flica_partB(Infolder=None, output_dir = os.path.join(os.getcwd(), "Flica_Output"),
    ...
    handle=open('dict_Y.pickle','rb')
    dict_Y=pickle.load(handle)
    handle1=open('dict_images_info.pickle','rb')
    dict_images_info=pickle.load(handle1)
    handle2=open('dict_Priors.pickle','rb')
    dict_Priors=pickle.load(handle2)
    handle3=open('dict_Posteriors.pickle','rb')
    dict_Posteriors=pickle.load(handle3)
    handle4=open('dict_Constants.pickle','rb')
    dict_Constants=pickle.load(handle4)

    Y=dict_Y["Y"]
    images_info=dict_images_info["images_info"]
    Priors=dict_Priors["Priors"]
    Posteriors=dict_Posteriors["Posteriors"]
    Constants=dict_Constants["Constants"]
