...
def flica_load(opts):
    ...
    for i in range (0,size(paths2data)):
        ...
        if fileExtension == '.gz': # should be .nii.gz, check ....
            ...
            data = img.get_data() 
            data2d=np.reshape(data,[data.shape[0]* data.shape[1]* data.shape[2],data.shape[3]], order='F')
            Data_Modality[i]=data2d[~np.all(data2d == 0, axis=1)]
            
            #gather info needed for saving
            affine[i]=img.affine
            header[i]=img.header
            Non_zero_voxels[i]= ~np.all(data2d == 0, axis=1)
            
            #masking
            tmp_mask=np.zeros(data2d.shape[0]); del data2d
            tmp_mask[find(Non_zero_voxels[i])]=1
            tmp_mask=np.reshape(tmp_mask, [data.shape[0], data.shape[1], data.shape[2]], order='F')
            mask[i]=tmp_mask
            del data;  
